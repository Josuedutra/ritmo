...
export async function calculateOrgMetricsForDate(
  ...
  try {
    ...
    const followUpRate = totalActions > 0 ? (completedActions / totalActions) * 100 : 0;
...
export async function calculateAllOrgMetricsForDate(date: Date): Promise<{
  ...
  logger.info({ date: date.toISOString(), processed, errors }, "All org metrics calculated");
...
export async function getScoreboardData(organizationId: string): Promise<{
  ...
  const overallFollowUpRate =
    totals.totalActions > 0 ? (totals.completedActions / totals.totalActions) * 100 : 0;
 ...
 */
export async function getBenchmarkData(organizationId: string): Promise<{
  sector: string | null;
  sampleSize: number;
