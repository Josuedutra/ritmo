...
export function calculateEntitlements(
  ...
  if (hasActiveSubscription) {
    ...
    storageQuotaBytes =
      PLAN_LIMITS[planKey]?.storageQuotaBytes ?? PLAN_LIMITS.starter.storageQuotaBytes;
  ...
  const trialBccCapturesRemaining =
    tier === "trial"
...
export async function checkTrialBccCapture(organizationId: string): Promise<{
  ...
  if (entitlements.trialBccCapturesUsed >= TRIAL_BCC_INBOUND_LIMIT) {
    return {
      ...
      message:
        "Já utilizou a captura automática incluída no trial. Atualize para continuar a usar.",
    };
  }
...
export async function checkAndIncrementTrialBccCapture(organizationId: string): Promise<
  ...
  | {
      ...
      isFirstCapture: false;
      committed: false;
      reason: "TRIAL_LIMIT_REACHED" | "FREE_TIER" | "PAID_TIER" | "ORG_NOT_FOUND";
      message: string;
    }
> {
...
export async function reserveStorageQuota(
  ...
  | {
      reserved: false;
      reason: "QUOTA_EXCEEDED" | "ORG_NOT_FOUND";
      currentUsed: number;
      quota: number;
    }
> {
