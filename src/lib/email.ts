...
export async function sendCadenceEmail(params: SendEmailParams): Promise<SendEmailResult> {
  ...
  if (org && !isWithinSendWindow(org.timezone, org.sendWindowStart, org.sendWindowEnd)) {
    log.info(
      { to: maskEmail(to), organizationId, timezone: org.timezone },
      "Outside send window - deferred"
    );
    return {
      success: false,
      deferred: true,
  ...
  const htmlWithOptOut = html
    ? html +
      `<br><br><hr><p style="font-size:12px;color:#666;">Para deixar de receber estes emails: <a href="${optOutLink}">Cancelar subscrição</a></p>`
    : undefined;
