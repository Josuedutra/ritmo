...
export function validateOpsToken(request: NextRequest): NextResponse | null {
  ...
  if (!expectedToken) {
    return NextResponse.json({ error: "OPS_TOKEN not configured" }, { status: 500 });
  }

  const providedToken = request.headers.get(OPS_TOKEN_HEADER);

  if (!providedToken) {
    return NextResponse.json({ error: "Missing x-ops-token header" }, { status: 401 });
  }

  // Timing-safe comparison (simple version)
  if (providedToken !== expectedToken) {
    return NextResponse.json({ error: "Invalid x-ops-token" }, { status: 401 });
  }
