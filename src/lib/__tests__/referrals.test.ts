...
test("Expired cookie doesn't create attribution", () => {
  ...
  const daysSinceCapture =
    (now.getTime() - new Date(cookiePayload.capturedAt).getTime()) / (1000 * 60 * 60 * 24);
  const isValid = daysSinceCapture <= 30;
...
test("Same invoice doesn't duplicate booster", () => {
  ...
  const existingBoosters = [{ stripeInvoiceId: "inv_123", amountCents: 585 }];
...
test("Org with existing booster doesn't get second booster", () => {
  ...
  const existingOrgBoosters = [{ organizationId: "org_123", stripeInvoiceId: "inv_001" }];
  const orgId = "org_123";
