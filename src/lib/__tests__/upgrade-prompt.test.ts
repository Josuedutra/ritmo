...
describe("runUpgradeCta", () => {
  ...
  it("should try checkout when portal returns choose_plan action", async () => {
    ...
    const checkoutCall = (global.fetch as ReturnType<typeof vi.fn>).mock.calls[1];
...
describe("Dedupe localStorage logic", () => {
  ...
  it("should return false when no previous shown event", () => {
    ...
    expect(hasShownRecently(storage, "send_limit", "quote_actions", now)).toBe(false);
  });
  ...
  it("should return true when shown within 24h window", () => {
    ...
    storage.set("upgrade_prompt_shown:send_limit:quote_actions", twelveHoursAgo.toString());

    expect(hasShownRecently(storage, "send_limit", "quote_actions", now)).toBe(true);
  });
  ...
  it("should return false when shown more than 24h ago", () => {
    ...
    storage.set("upgrade_prompt_shown:send_limit:quote_actions", thirtyHoursAgo.toString());
  ...
  it("should use different keys for different reason+location combos", () => {
    ...
    storage.set("upgrade_prompt_shown:send_limit:quote_actions", recentTime.toString());

    // Same reason, different location should NOT be deduped
    expect(hasShownRecently(storage, "send_limit", "proposal_section", now)).toBe(false);

    // Different reason, same location should NOT be deduped
    expect(hasShownRecently(storage, "storage_quota", "quote_actions", now)).toBe(false);
