...
export function ReferralCodeForm() {
  ...
  const validate = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/referrals/capture?code=${encodeURIComponent(trimmed)}`);
    ...
    <div className="mx-auto w-full max-w-md">
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-background)] px-4 py-3 text-sm placeholder:text-[var(--color-muted-foreground)] focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:outline-none"
          disabled={loading}
          autoComplete="off"
        />
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-[var(--color-primary)] px-5 py-3 text-sm font-medium text-white transition-colors hover:bg-[var(--color-primary)]/90 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-900 dark:bg-green-950/30">
          <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
            ...
            <p className="text-sm font-medium">
              Código válido — parceiro <strong>{result.partnerName}</strong>
            </p>
          </div>
