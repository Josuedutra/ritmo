...
export function ReferralsAdminClient() {
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const filename =
        res.headers.get("Content-Disposition")?.match(/filename="(.+)"/)?.[1] || "boosters.csv";
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {attributions.filter((a) => a.status === "CONVERTED").length}
                </p>
                ...
                <tbody>
                  {links.map((link) => (
                    <tr key={link.id} className="border-b">
                      ...
                      <td className="max-w-xs truncate py-3 text-xs text-[var(--color-muted-foreground)]">
                        {link.fullUrl}
                      </td>
                  ...
                  {attributions.map((attr) => (
                    <tr key={attr.id} className="border-b">
                      ...
                      <td className="py-3 text-xs">
                        {attr.signupAt ? formatDate(attr.signupAt) : "-"}
                      </td>
                      <td className="py-3 text-xs">
                        {attr.convertedAt ? formatDate(attr.convertedAt) : "-"}
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  {boosters.map((booster) => (
                    <tr key={booster.id} className="border-b">
                      ...
                      <td className="py-3 font-medium">
                        {formatCurrency(booster.amountCents, booster.currency)}
                      </td>
