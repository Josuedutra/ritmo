          ...
          {cron ? (
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span
                  className={`font-mono ${
                    cron.cron.purgeProposals.lastRun24h ? "text-green-600" : "text-yellow-600"
                  }`}
                >
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono">{cron.cron.purgeProposals.purgedLast24h}</span>
              </div>
              <div className="flex justify-between">
                <span>Pending</span>
                <span
                  className={`font-mono ${
                    cron.cron.purgeProposals.pendingPurge > 1000 ? "text-red-600" : ""
                  }`}
                >
              ...
              {cron.cron.purgeProposals.lastSuccess && (
                <div className="border-t pt-2 text-xs">
                  <span className="text-[var(--color-muted-foreground)]">Last success: </span>
                  <span>{new Date(cron.cron.purgeProposals.lastSuccess).toLocaleString()}</span>
                </div>
              )}
            </div>
          ) : (
      ...
      {metrics && (
        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] p-4">
          ...
          <div className="grid gap-2 text-sm md:grid-cols-4">
            <div>
              <span className="text-[var(--color-muted-foreground)]">Inbound Rejection:</span> {">"}{" "}
              {metrics.thresholds.inboundRejectionRate}%
            </div>
            <div>
              <span className="text-[var(--color-muted-foreground)]">Stripe Failures:</span> {">"}{" "}
        ...
        {sentryResult && (
          <div className="mt-4 rounded border border-green-500/30 bg-green-500/10 p-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-green-600">Evento enviado com sucesso</span>
            </div>
            <div className="mt-2 space-y-1 text-xs">
              <div className="flex gap-2">
                <span className="text-[var(--color-muted-foreground)]">Event ID:</span>
                <code className="font-mono text-green-600">{sentryResult.sentryEventId}</code>
              </div>
              <div className="flex gap-2">
