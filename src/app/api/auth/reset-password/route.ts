...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!token || typeof token !== "string") {
      return NextResponse.json({ error: "Token inválido ou em falta" }, { status: 400 });
    }
    ...
    if (resetToken.usedAt) {
      log.warn({ tokenId: resetToken.id, userId: resetToken.userId }, "Reset token already used");
      return NextResponse.json({ error: "Este link já foi utilizado" }, { status: 400 });
    }

    // Check if token is expired
    if (resetToken.expiresAt < new Date()) {
      log.warn({ tokenId: resetToken.id, userId: resetToken.userId }, "Reset token expired");
      return NextResponse.json(
    ...
    return NextResponse.json({ error: "Ocorreu um erro ao repor a password" }, { status: 500 });
  }
}
