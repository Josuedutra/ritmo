...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!emailRateLimitResult.allowed) {
      log.warn({ email: normalizedEmail }, "Forgot password: email rate limited (possible abuse)");
    ...
    if (recentToken) {
      const secondsAgo = Math.floor((Date.now() - recentToken.createdAt.getTime()) / 1000);
      const waitSeconds = EMAIL_COOLDOWN_SECONDS - secondsAgo;
      log.debug({ email: normalizedEmail, waitSeconds }, "Forgot password: cooldown active");
    ...
    try {
      ...
      log.info({ userId: user.id, email: normalizedEmail }, "Password reset email sent");
    } catch (emailError) {
      // Log but don't fail the request (user gets generic success message)
      log.error({ error: emailError, userId: user.id }, "Failed to send password reset email");
    }
