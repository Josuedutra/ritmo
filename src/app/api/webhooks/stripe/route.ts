  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      ...
      if (existingEvent.status === "PROCESSED") {
        log.info({ eventId: event.id }, "Event already processed - skipping (idempotent)");
      ...
      if (existingEvent.status === "PROCESSING") {
        ...
        log.warn({ eventId: event.id, ageMs }, "Taking over stale processing event");
        await prisma.stripeEvent.update({
          where: { stripeEventId: event.id },
          data: {
            claimedAt: new Date(),
  ...
  try {
    switch (event.type) {
      ...
      case "customer.subscription.created":
        await handleSubscriptionCreated(event.data.object as Stripe.Subscription, log);
        break;
...
// =============================================================================
async function handleCheckoutCompleted(session: Stripe.Checkout.Session, log: AppLogger) {
  const { organizationId, planId } = session.metadata || {};

  if (!organizationId) {
    log.warn({ sessionId: session.id }, "Checkout session missing organizationId in metadata");
    return;
  }
  ...
  if (!stripeCustomerId || !stripeSubscriptionId) {
    log.warn({ sessionId: session.id }, "Checkout session missing customer or subscription");
    return;
  }
...
// =============================================================================
async function handleSubscriptionCreated(subscription: Stripe.Subscription, log: AppLogger) {
...
// =============================================================================
async function handleSubscriptionUpdated(subscription: Stripe.Subscription, log: AppLogger) {
  ...
  if (!existingSubscription) {
    log.warn({ stripeSubscriptionId: subscription.id }, "No subscription found for update");
    return;
  }
...
// =============================================================================
async function handleSubscriptionDeleted(subscription: Stripe.Subscription, log: AppLogger) {
  const existingSubscription = await prisma.subscription.findFirst({
    where: { stripeSubscriptionId: subscription.id },
  });

  if (!existingSubscription) {
    log.warn({ stripeSubscriptionId: subscription.id }, "No subscription found for deletion");
    return;
  }
...
// =============================================================================
async function handleInvoicePaymentSucceeded(invoice: Stripe.Invoice, log: AppLogger) {
  ...
  if (!existingSubscription) {
    log.warn({ stripeSubscriptionId: invoice.subscription }, "No subscription found for invoice");
    return;
  }
  ...
  if (existingSubscription.status !== "active") {
    ...
    log.info({ organizationId }, "Subscription reactivated after payment");
  }
  ...
  // =========================================================================
  try {
    await handleReferralBooster(organizationId, existingSubscription.id, invoice, log);
  } catch (error) {
...
async function handleReferralBooster(
  ...
  if (existingBooster) {
    log.debug({ invoiceId: invoice.id }, "Booster already exists for this invoice - skipping");
    return;
  }
...
// =============================================================================
async function handleInvoicePaymentFailed(invoice: Stripe.Invoice, log: AppLogger) {
  if (!invoice.subscription) {
    log.debug("Invoice not related to subscription - skipping");
    return;
  }
 ...
 */
async function getPlanIdFromSubscription(subscription: Stripe.Subscription): Promise<string> {
