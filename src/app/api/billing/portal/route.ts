...
export async function POST() {
  ...
  try {
    ...
    if (session.user.role !== "admin") {
      log.warn({ userId: session.user.id }, "Non-admin attempted to access billing portal");
    ...
    log.info({ organizationId: session.user.organizationId }, "Portal session created");
