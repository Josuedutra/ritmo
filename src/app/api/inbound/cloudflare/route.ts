...
function verifyCloudflareSignature(
  ...
  if (!CLOUDFLARE_INBOUND_SECRET) {
    log.warn(
      "CLOUDFLARE_INBOUND_SECRET not configured - skipping signature verification (dev mode)"
    );
...
function generateIdempotencyKey(messageId: string | null, from: string, timestamp: string): string {
  ...
  return `cf:${createHash("sha256").update(`${from}:${timestamp}`).digest("hex").substring(0, 32)}`;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!bccParsed) {
      ...
      log.warn(
        { id: ingestion.id, recipients: to?.substring(0, 100) },
    ...
    if (!bccEnabled) {
      ...
      log.info(
        { id: ingestion.id, orgShortId },
        "Inbound rejected - BCC feature disabled (free tier)"
      );
