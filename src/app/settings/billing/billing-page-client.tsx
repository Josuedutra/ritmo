...
export function BillingPageClient({ data }: BillingPageClientProps) {
  ...
  const handleCheckout = async (planId: string) => {
    ...
    if (!PAYMENTS_ENABLED) {
      toast({
        ...
        description:
          "O sistema de pagamentos ainda não está disponível. Continuamos a trabalhar para o ativar em breve.",
      });
      return;
    }
  ...
  const getStatusBadge = () => {
    ...
    if (isCancelAtPeriodEnd)
      return { text: "Termina no fim do período", variant: "warning" as const };
  ...
  const getUsageThresholdText = () => {
    ...
    if (usagePercentage >= 90) {
      return {
        text: "Quase no limite — considere upgrade para não parar.",
  ...
  const getPlanButtonText = (plan: Plan, isCurrent: boolean): { full: string; short: string } => {
    ...
    if (plan.priceMonthly < currentPlanPrice)
      return { full: "Fazer downgrade", short: "Downgrade" };
      ...
      {success && (
        <Card className="border-success bg-success">
          <CardContent className="flex items-center gap-3 py-4">
            ...
            <p className="text-success text-sm">Plano ativado com sucesso.</p>
          </CardContent>
        </Card>
      )}
      ...
      {canceled && (
        <Card className="border-warning bg-warning">
          <CardContent className="flex items-center gap-3 py-4">
            ...
            <p className="text-warning text-sm">Checkout cancelado. Pode retomar quando quiser.</p>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-destructive bg-destructive-subtle">
          <CardContent className="flex items-center justify-between py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-destructive font-medium">Pagamento em atraso</p>
                <p className="text-destructive text-sm">
                  Atualize o método de pagamento para continuar a enviar orçamentos e follow-ups.
                </p>
              </div>
            </div>
            <Button onClick={handlePortal} disabled={loading === "portal"} variant="destructive">
              {loading === "portal" && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Atualizar pagamento
            </Button>
          </CardContent>
        ...
        <Card className="border-[var(--color-border)] bg-[var(--color-muted)]">
          <CardContent className="flex items-center justify-between py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-foreground font-medium">Subscrição cancelada</p>
        ...
        <Card className="border-warning bg-warning">
          <CardContent className="flex items-center justify-between py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-warning font-medium">Plano a terminar</p>
                <p className="text-warning text-sm">
                  O seu plano termina em {formatDate(subscription?.currentPeriodEnd ?? null)}. Pode
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="bg-muted/50 flex items-start gap-2 rounded p-3 text-sm">
            ...
            {subscription?.hasStripeCustomer ? (
              ...
              <Button
                ...
                className="w-full"
                onClick={() =>
                  (window.location.href = "mailto:ritmo@useritmo.pt?subject=Plano%20Early%20Access")
                }
              >
                Contactar suporte
              </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Threshold text */}
            {usageThreshold && (
              <p className={`text-sm font-medium ${usageThreshold.color}`}>{usageThreshold.text}</p>
            )}

            {/* Helper text for trial/free */}
            {usageHelper && <p className="text-muted-foreground text-xs">{usageHelper}</p>}
            ...
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--color-info)]">
                  Estamos a preparar o sistema de pagamentos. Por agora, pode continuar a usar o
                  trial ou o plano gratuito.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {plans.map((plan) => {
            ...
            return (
              <Card
                key={plan.id}
                className={`relative ${isCurrent ? "border-primary ring-primary ring-1" : ""}`}
              >
                {isCurrent && (
