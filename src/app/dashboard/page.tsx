...
function UsageMeter({ entitlements }: { entitlements: Entitlements }) {
  ...
  const percentUsed =
    effectivePlanLimit > 0 ? Math.round((quotesUsed / effectivePlanLimit) * 100) : 0;

  // P0-05: More descriptive tier labels
  const tierLabel = tier === "trial" ? "Trial" : tier === "paid" ? planName : "Gratuito";

  // P0-05: Subtitle with context
  const tierSubtitle =
    tier === "trial" && trialDaysRemaining
  ...
  return (
    <Card
      className={
        percentUsed >= 100 ? "border-red-500/50" : percentUsed >= 80 ? "border-orange-500/50" : ""
      }
    >
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {tierSubtitle && (
              <span className="text-xs text-[var(--color-muted-foreground)]">{tierSubtitle}</span>
            )}
          </div>
        </div>
      </CardHeader>
      <CardContent>
        ...
        {percentUsed >= 100 ? (
          ...
          <div className="mt-3 rounded-md bg-orange-500/10 p-2">
            <p className="text-xs text-orange-500">{effectivePlanLimit - quotesUsed} restantes</p>
            <Link
              href="/settings/billing"
