...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-[var(--color-border)] bg-transparent px-3 text-sm font-medium transition-colors hover:bg-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-ring)] focus:outline-none disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute top-2 right-2 rounded-md p-1 text-[var(--color-foreground)]/50 opacity-0 transition-opacity group-hover:opacity-100 hover:text-[var(--color-foreground)] focus:opacity-100 focus:ring-2 focus:outline-none",
      className
    )}
    toast-close=""
    {...props}
...
// Toast icon helper
function ToastIcon({ variant }: { variant?: "default" | "success" | "error" | "warning" }) {
  switch (variant) {
    case "success":
    ...
    default:
      return <Info className="h-5 w-5 text-[var(--color-muted-foreground)]" />;
  }
}
