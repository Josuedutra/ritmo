...
import { FileText, PlusCircle, Search, type LucideIcon } from "lucide-react";
...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <h3 className="mb-1 text-sm font-medium text-[var(--color-foreground)]">{title}</h3>

      {description && (
        <p className="mb-4 max-w-sm text-sm text-[var(--color-muted-foreground)]">{description}</p>
      )}
