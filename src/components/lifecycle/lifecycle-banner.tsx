...
export function LifecycleBanner() {
  ...
  if (entitlements.tier === "trial" && entitlements.trialDaysRemaining !== null) {
    ...
    const hasBccLimit =
      entitlements.trialBccCaptureLimit > 0 && entitlements.trialBccCapturesRemaining === 0;
    ...
    const linkColor = isUrgent
      ...
      : "text-[var(--color-info)] hover:text-[var(--color-info-foreground)]";
    ...
    return (
      <div className={`mb-4 rounded-lg border ${borderColor} ${bgColor} px-4 py-3`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className={`text-sm font-medium ${textColor}`}>
                ...
                {entitlements.trialDaysRemaining !== 1 ? "s" : ""} · {entitlements.quotesUsed}/
                {entitlements.quotesLimit} envios
                {hasBccLimit && (
          ...
          <button onClick={() => setDismissed(true)} className={linkColor}>
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
