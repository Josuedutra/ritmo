...
async function runTests() {
  ...
  if (PLAN_LIMITS.enterprise.storageQuotaBytes === 50 * 1024 * 1024 * 1024) {
    ...
    fail(
      "Enterprise tier quota",
      `Expected 50 GB, got ${PLAN_LIMITS.enterprise.storageQuotaBytes}`
    );
  }
  ...
  if (trialEntitlements.storageQuotaBytes === PLAN_LIMITS.starter.storageQuotaBytes) {
    ...
    fail(
      "Trial storage quota",
      `Expected ${PLAN_LIMITS.starter.storageQuotaBytes}, got ${trialEntitlements.storageQuotaBytes}`
    );
  }
  ...
  if (trialEntitlements.retentionDays === PLAN_LIMITS.starter.retentionDays) {
    ...
    fail(
      "Trial retention",
      `Expected ${PLAN_LIMITS.starter.retentionDays}, got ${trialEntitlements.retentionDays}`
    );
  }
  ...
  if (freeEntitlements.storageQuotaBytes === PLAN_LIMITS.free.storageQuotaBytes) {
    ...
    fail(
      "Free storage quota",
      `Expected ${PLAN_LIMITS.free.storageQuotaBytes}, got ${freeEntitlements.storageQuotaBytes}`
    );
  }
  ...
  if (freeEntitlements.retentionDays === PLAN_LIMITS.free.retentionDays) {
    ...
    fail(
      "Free retention",
      `Expected ${PLAN_LIMITS.free.retentionDays}, got ${freeEntitlements.retentionDays}`
    );
  }
