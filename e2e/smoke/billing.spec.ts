...
test.describe("Billing Page", () => {
  ...
  test("should display available plans", async ({ page }) => {
    ...
    const plansVisible = await page
      .getByText(/starter|pro|business|free|trial/i)
      .first()
      .isVisible();
    const upgradeVisible = await page
      .getByRole("button", { name: /upgrade|atualizar|escolher|select/i })
      .first()
      .isVisible();
  ...
  test("should show usage progress", async ({ page }) => {
    ...
    const hasUsageText = await usageText
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test("should handle manage subscription click", async ({ page }) => {
    ...
    const manageButton = page.getByRole("button", {
      name: /gerir.*subscrição|manage.*subscription|portal/i,
    });
  ...
  test("should handle checkout flow for plan upgrade", async ({ page }) => {
    ...
    const upgradeButton = page
      .getByRole("button", { name: /escolher|select|upgrade|atualizar/i })
      .first();
  ...
  test("should show success message after returning from checkout", async ({ page }) => {
    ...
    await expect(page.getByText(/sucesso|success|ativad|activated/i)).toBeVisible({
      timeout: 5000,
    });
  });
